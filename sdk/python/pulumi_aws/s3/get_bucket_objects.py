...
def get_bucket_objects(bucket=None, delimiter=None, encoding_type=None, fetch_owner=None, max_keys=None, prefix=None, start_after=None, opts=None):
    ...
    my_objects = aws.s3.get_bucket_objects(aws.s3.GetBucketObjectsArgsArgs(
        bucket="ourcorp",
    ))
    object_info = [aws.s3.get_bucket_object(aws.s3.GetBucketObjectArgsArgs(
        bucket=my_objects.bucket,
        key=my_objects.keys[__index],
    )) for __index in range(len(my_objects.keys))]
    ```
