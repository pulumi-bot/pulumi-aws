...
class WebAcl(pulumi.CustomResource):
    ...
    Configuration blocks containing rules to associate with the web ACL and the settings for each rule. Detailed below.
      ...
      * `override_action` (`dict`) - Override the action that a group requests CloudFront or AWS WAF takes when a web request matches the conditions in the rule. Only used if `type` is `GROUP`.
    ...
    def __init__(__self__, resource_name, opts=None, default_action=None, logging_configuration=None, metric_name=None, name=None, rules=None, __props__=None, __name__=None, __opts__=None):
        ...
        The **rules** object supports the following:
          ...
          * `override_action` (`pulumi.Input[dict]`) - Override the action that a group requests CloudFront or AWS WAF takes when a web request matches the conditions in the rule. Only used if `type` is `GROUP`.
