...
class Fleet(pulumi.CustomResource):
    ...
    Range of IP addresses and port settings that permit inbound traffic to access server processes running on the fleet. See below.
      ...
      * `ip_range` (`str`) - Range of allowed IP addresses expressed in CIDR notation. e.g. `000.000.000.000/[subnet mask]` or `0.0.0.0/[subnet mask]`.
    ...
    Policy that limits the number of game sessions an individual player can create over a span of time for this fleet. See below.
      ...
      * `new_game_sessions_per_creator` (`float`) - Maximum number of game sessions that an individual can create during the policy period.
      * `policy_period_in_minutes` (`float`) - Time span used in evaluating the resource creation limit policy.
    """
    runtime_configuration: pulumi.Output[dict]
    """
    Instructions for launching server processes on each instance in the fleet. See below.
    
      * `game_session_activation_timeout_seconds` (`float`) - Maximum amount of time (in seconds) that a game session can remain in status `ACTIVATING`.
      * `max_concurrent_game_session_activations` (`float`) - Maximum number of game sessions with status `ACTIVATING` to allow on an instance simultaneously. 
      * `server_processes` (`list`) - Collection of server process configurations that describe which server processes to run on each instance in a fleet. See below.
    
        * `concurrent_executions` (`float`) - Number of server processes using this configuration to run concurrently on an instance.
        * `launch_path` (`str`) - Location of the server executable in a game build. All game builds are installed on instances at the root : for Windows instances `C:\game`, and for Linux instances `/local/game`.
    ...
    def __init__(__self__, resource_name, opts=None, build_id=None, description=None, ec2_inbound_permissions=None, ec2_instance_type=None, metric_groups=None, name=None, new_game_session_protection_policy=None, resource_creation_limit_policy=None, runtime_configuration=None, __props__=None, __name__=None, __opts__=None):
        ...
        The **ec2_inbound_permissions** object supports the following:
          ...
          * `ip_range` (`pulumi.Input[str]`) - Range of allowed IP addresses expressed in CIDR notation. e.g. `000.000.000.000/[subnet mask]` or `0.0.0.0/[subnet mask]`.
        ...
        The **resource_creation_limit_policy** object supports the following:
          ...
          * `new_game_sessions_per_creator` (`pulumi.Input[float]`) - Maximum number of game sessions that an individual can create during the policy period.
          * `policy_period_in_minutes` (`pulumi.Input[float]`) - Time span used in evaluating the resource creation limit policy.
        
        The **runtime_configuration** object supports the following:
        
          * `game_session_activation_timeout_seconds` (`pulumi.Input[float]`) - Maximum amount of time (in seconds) that a game session can remain in status `ACTIVATING`.
          * `max_concurrent_game_session_activations` (`pulumi.Input[float]`) - Maximum number of game sessions with status `ACTIVATING` to allow on an instance simultaneously. 
          * `server_processes` (`pulumi.Input[list]`) - Collection of server process configurations that describe which server processes to run on each instance in a fleet. See below.
        
            * `concurrent_executions` (`pulumi.Input[float]`) - Number of server processes using this configuration to run concurrently on an instance.
            * `launch_path` (`pulumi.Input[str]`) - Location of the server executable in a game build. All game builds are installed on instances at the root : for Windows instances `C:\game`, and for Linux instances `/local/game`.
