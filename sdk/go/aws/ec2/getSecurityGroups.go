// *** WARNING: this file was generated by the Pulumi Terraform Bridge (tfgen) Tool. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

package ec2

import (
	"context"
	"reflect"

	"github.com/pulumi/pulumi/sdk/v3/go/pulumi"
)

// Use this data source to get IDs and VPC membership of Security Groups that are created
// outside of this provider.
func GetSecurityGroups(ctx *pulumi.Context, args *GetSecurityGroupsArgs, opts ...pulumi.InvokeOption) (*GetSecurityGroupsResult, error) {
	var rv GetSecurityGroupsResult
	err := ctx.Invoke("aws:ec2/getSecurityGroups:getSecurityGroups", args, &rv, opts...)
	if err != nil {
		return nil, err
	}
	return &rv, nil
}

// A collection of arguments for invoking getSecurityGroups.
type GetSecurityGroupsArgs struct {
	// One or more name/value pairs to use as filters. There are several valid keys, for a full reference, check out [describe-security-groups in the AWS CLI reference][1].
	Filters []GetSecurityGroupsFilter `pulumi:"filters"`
	// A map of tags, each pair of which must exactly match for desired security groups.
	Tags map[string]string `pulumi:"tags"`
}

// A collection of values returned by getSecurityGroups.
type GetSecurityGroupsResult struct {
	// ARNs of the matched security groups.
	Arns    []string                  `pulumi:"arns"`
	Filters []GetSecurityGroupsFilter `pulumi:"filters"`
	// The provider-assigned unique ID for this managed resource.
	Id string `pulumi:"id"`
	// IDs of the matches security groups.
	Ids  []string          `pulumi:"ids"`
	Tags map[string]string `pulumi:"tags"`
	// The VPC IDs of the matched security groups. The data source's tag or filter *will span VPCs* unless the `vpc-id` filter is also used.
	VpcIds []string `pulumi:"vpcIds"`
}

func GetSecurityGroupsApply(ctx *pulumi.Context, args GetSecurityGroupsApplyInput, opts ...pulumi.InvokeOption) GetSecurityGroupsResultOutput {
	return args.ToGetSecurityGroupsApplyOutput().ApplyT(func(v GetSecurityGroupsArgs) (GetSecurityGroupsResult, error) {
		r, err := GetSecurityGroups(ctx, &v, opts...)
		return *r, err

	}).(GetSecurityGroupsResultOutput)
}

// GetSecurityGroupsApplyInput is an input type that accepts GetSecurityGroupsApplyArgs and GetSecurityGroupsApplyOutput values.
// You can construct a concrete instance of `GetSecurityGroupsApplyInput` via:
//
//          GetSecurityGroupsApplyArgs{...}
type GetSecurityGroupsApplyInput interface {
	pulumi.Input

	ToGetSecurityGroupsApplyOutput() GetSecurityGroupsApplyOutput
	ToGetSecurityGroupsApplyOutputWithContext(context.Context) GetSecurityGroupsApplyOutput
}

// A collection of arguments for invoking getSecurityGroups.
type GetSecurityGroupsApplyArgs struct {
	// One or more name/value pairs to use as filters. There are several valid keys, for a full reference, check out [describe-security-groups in the AWS CLI reference][1].
	Filters GetSecurityGroupsFilterArrayInput `pulumi:"filters"`
	// A map of tags, each pair of which must exactly match for desired security groups.
	Tags pulumi.StringMapInput `pulumi:"tags"`
}

func (GetSecurityGroupsApplyArgs) ElementType() reflect.Type {
	return reflect.TypeOf((*GetSecurityGroupsArgs)(nil)).Elem()
}

func (i GetSecurityGroupsApplyArgs) ToGetSecurityGroupsApplyOutput() GetSecurityGroupsApplyOutput {
	return i.ToGetSecurityGroupsApplyOutputWithContext(context.Background())
}

func (i GetSecurityGroupsApplyArgs) ToGetSecurityGroupsApplyOutputWithContext(ctx context.Context) GetSecurityGroupsApplyOutput {
	return pulumi.ToOutputWithContext(ctx, i).(GetSecurityGroupsApplyOutput)
}

// A collection of arguments for invoking getSecurityGroups.
type GetSecurityGroupsApplyOutput struct{ *pulumi.OutputState }

func (GetSecurityGroupsApplyOutput) ElementType() reflect.Type {
	return reflect.TypeOf((*GetSecurityGroupsArgs)(nil)).Elem()
}

func (o GetSecurityGroupsApplyOutput) ToGetSecurityGroupsApplyOutput() GetSecurityGroupsApplyOutput {
	return o
}

func (o GetSecurityGroupsApplyOutput) ToGetSecurityGroupsApplyOutputWithContext(ctx context.Context) GetSecurityGroupsApplyOutput {
	return o
}

// One or more name/value pairs to use as filters. There are several valid keys, for a full reference, check out [describe-security-groups in the AWS CLI reference][1].
func (o GetSecurityGroupsApplyOutput) Filters() GetSecurityGroupsFilterArrayOutput {
	return o.ApplyT(func(v GetSecurityGroupsArgs) []GetSecurityGroupsFilter { return v.Filters }).(GetSecurityGroupsFilterArrayOutput)
}

// A map of tags, each pair of which must exactly match for desired security groups.
func (o GetSecurityGroupsApplyOutput) Tags() pulumi.StringMapOutput {
	return o.ApplyT(func(v GetSecurityGroupsArgs) map[string]string { return v.Tags }).(pulumi.StringMapOutput)
}

// A collection of values returned by getSecurityGroups.
type GetSecurityGroupsResultOutput struct{ *pulumi.OutputState }

func (GetSecurityGroupsResultOutput) ElementType() reflect.Type {
	return reflect.TypeOf((*GetSecurityGroupsResult)(nil)).Elem()
}

func (o GetSecurityGroupsResultOutput) ToGetSecurityGroupsResultOutput() GetSecurityGroupsResultOutput {
	return o
}

func (o GetSecurityGroupsResultOutput) ToGetSecurityGroupsResultOutputWithContext(ctx context.Context) GetSecurityGroupsResultOutput {
	return o
}

// ARNs of the matched security groups.
func (o GetSecurityGroupsResultOutput) Arns() pulumi.StringArrayOutput {
	return o.ApplyT(func(v GetSecurityGroupsResult) []string { return v.Arns }).(pulumi.StringArrayOutput)
}

func (o GetSecurityGroupsResultOutput) Filters() GetSecurityGroupsFilterArrayOutput {
	return o.ApplyT(func(v GetSecurityGroupsResult) []GetSecurityGroupsFilter { return v.Filters }).(GetSecurityGroupsFilterArrayOutput)
}

// The provider-assigned unique ID for this managed resource.
func (o GetSecurityGroupsResultOutput) Id() pulumi.StringOutput {
	return o.ApplyT(func(v GetSecurityGroupsResult) string { return v.Id }).(pulumi.StringOutput)
}

// IDs of the matches security groups.
func (o GetSecurityGroupsResultOutput) Ids() pulumi.StringArrayOutput {
	return o.ApplyT(func(v GetSecurityGroupsResult) []string { return v.Ids }).(pulumi.StringArrayOutput)
}

func (o GetSecurityGroupsResultOutput) Tags() pulumi.StringMapOutput {
	return o.ApplyT(func(v GetSecurityGroupsResult) map[string]string { return v.Tags }).(pulumi.StringMapOutput)
}

// The VPC IDs of the matched security groups. The data source's tag or filter *will span VPCs* unless the `vpc-id` filter is also used.
func (o GetSecurityGroupsResultOutput) VpcIds() pulumi.StringArrayOutput {
	return o.ApplyT(func(v GetSecurityGroupsResult) []string { return v.VpcIds }).(pulumi.StringArrayOutput)
}

func init() {
	pulumi.RegisterOutputType(GetSecurityGroupsApplyOutput{})
	pulumi.RegisterOutputType(GetSecurityGroupsResultOutput{})
}
