...
import (
	...
	"https:/github.com/pulumi/pulumi-aws/rds/ClusterS3Import"
	"https:/github.com/pulumi/pulumi-aws/rds/ClusterScalingConfiguration"
)
...
type Cluster struct {
	...
	S3Import rdsClusterS3Import.ClusterS3ImportPtrOutput `pulumi:"s3Import"`
	// Nested attribute with scaling properties. Only valid when `engineMode` is set to `serverless`. More details below.
	ScalingConfiguration rdsClusterScalingConfiguration.ClusterScalingConfigurationPtrOutput `pulumi:"scalingConfiguration"`
...
type clusterState struct {
	...
	S3Import *rdsClusterS3Import.ClusterS3Import `pulumi:"s3Import"`
	// Nested attribute with scaling properties. Only valid when `engineMode` is set to `serverless`. More details below.
	ScalingConfiguration *rdsClusterScalingConfiguration.ClusterScalingConfiguration `pulumi:"scalingConfiguration"`
	// Determines whether a final DB snapshot is created before the DB cluster is deleted. If true is specified, no DB snapshot is created. If false is specified, a DB snapshot is created before the DB cluster is deleted, using the value from `finalSnapshotIdentifier`. Default is `false`.
...
type ClusterState struct {
	...
	S3Import rdsClusterS3Import.ClusterS3ImportPtrInput
	// Nested attribute with scaling properties. Only valid when `engineMode` is set to `serverless`. More details below.
	ScalingConfiguration rdsClusterScalingConfiguration.ClusterScalingConfigurationPtrInput
	// Determines whether a final DB snapshot is created before the DB cluster is deleted. If true is specified, no DB snapshot is created. If false is specified, a DB snapshot is created before the DB cluster is deleted, using the value from `finalSnapshotIdentifier`. Default is `false`.
